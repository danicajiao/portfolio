...
const getProjectElements = (projectId) => {
  ...
  if (!bg) {
    console.warn(
      `[getProjectElements] Missing background element for project: "${projectId}"`,
    );
  }

  if (!leftPanel) {
    console.warn(
      `[getProjectElements] Missing left panel element for project: "${projectId}"`,
    );
  }

  if (!rightPanel) {
    console.warn(
      `[getProjectElements] Missing right panel element for project: "${projectId}"`,
    );
  }
...
const getSchwabSpecificElements = () => {
  ...
  const inProgressIndicators = document.querySelectorAll(
  ...
  if (inProgressIndicators.length === 0) {
    console.warn(
      '[getSchwabSpecificElements] No in-progress-indicator elements found',
    );
  }

  if (successIndicators.length === 0) {
    console.warn(
      '[getSchwabSpecificElements] No success-indicator elements found',
    );
  }
...
export const animateProject = (projectId, action, config) => {
  ...
  if (!animations) {
    console.warn(
      `[animateProject] No animations defined for project: "${projectId}"`,
    );
    return;
  }
 ...
 */
export const hasProjectAnimations = (projectId) =>
  projectId in PROJECT_ANIMATIONS;
